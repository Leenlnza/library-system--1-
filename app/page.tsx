...
// อัปเดต Interface ให้ตรงกับข้อมูลที่เก็บใน localStorage (จาก JWT)
interface Member \{
  id: string
  name: string
  email: string
  phone?: string // อาจจะไม่มีใน JWT payload แต่มีในข้อมูลสมาชิกเต็ม
\}
...
\
{
  ...
  const [history, setHistory] = useState<BorrowHistory[]>([]) // ยังคงเก็บ history ใน state เพื่อใช้ใน borrowBook/returnBook
  const [searchTerm, setSearchTerm] = useState("")
  const [selectedBook, setSelectedBook] = useState<Book | null>(null)
  const [currentMember, setCurrentMember] = useState<Member | null>(null) // ดึงข้อมูลสมาชิกปัจจุบันจาก localStorage
  const router = useRouter()

  // โหลดข้อมูลหนังสือและสมาชิกปัจจุบัน
  useEffect(() => \{
  ...
  setBooks(
    data.map((b: any) => (\{\
          ...
          borrowerPhone: b.borrowerPhone, // เพิ่ม borrowerPhone\
...
const borrowBook = async (book: Book) => \
{
  ...
  \
  {
    const response = await fetch(`/api/books/$\{book.id\}/borrow`, \{
        ...
        body: JSON.stringify(\{\
          ...
          borrowerPhone: currentMember.phone, // ส่งเบอร์โทรไปด้วย
        \}),
    \
  }
  )
  ...
  \
      setBooks(updatedBooksData.map((b: any) => (\
  \
        id: b._id,\
        title: b.title,
        author: b.author,
...
const returnBook = async (book: Book) => \
{
  ...
  \
  {
    const response = await fetch(`/api/books/$\{book.id\}/return`, \{
        ...
        \},
        body: JSON.stringify(\{
          borrowerName: currentMember.name, // ใช้ชื่อสมาชิกที่เข้าสู่ระบบ
          borrowerPhone: currentMember.phone, // ใช้เบอร์โทรสมาชิกที่เข้าสู่ระบบ
        \}),
    \
  }
  )
